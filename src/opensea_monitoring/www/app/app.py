...
with col1:
    comps.linear_plot("transfers_count", grain)
    comps.multilinear_plot("transfers_count", grain)
with col2:
    df = pl.DataFrame(
        comps.get_metric("transfers_count", "1 minute"),
    )
    ...
    comps.render_as_table(
        "transfers_count", grain, n=10, collection_href_col="collection"
    )
